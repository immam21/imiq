...
def setup_google_credentials():
    ...
    # Check Streamlit secrets for local development
    try:
        if hasattr(st, 'secrets') and 'GOOGLE_SERVICE_ACCOUNT' in st.secrets:
            ...
            with tempfile.NamedTemporaryFile(mode='w', suffix='.json', delete=False) as temp_file:
                ...
                print(f"✅ Using Google credentials from Streamlit secrets")
                return True
...
def main():
    ...
    if not st.session_state.authenticated:
        ...
        # Check if we should redirect to login after successful signup
        redirect_to_login = st.session_state.get('redirect_to_login', False)
        
        # If redirecting to login, show login content in both tabs
        if redirect_to_login:
            # Show login form in both tabs with success message
            for i, tab in enumerate([tab1, tab2]):
                with tab:
                    st.success("✅ Account created successfully!")
                    st.info("Please login with your new credentials:")
                    
                    # Show account details for reference
                    if 'new_account_details' in st.session_state:
                        details = st.session_state['new_account_details']
                        st.write(f"**User ID:** `{details['user_id']}`")
                    
                    with st.form(f"login_form_{i}"):
                        user_id = st.text_input("User ID", placeholder="Enter your user ID", 
                                               value=st.session_state.get('new_account_details', {}).get('user_id', ''),
                                               key=f"user_id_{i}")
                        ...
                        if user_id and password:
                            try:
                                ...
                                if user:
                                    ...
                                    st.session_state.pop('new_account_details', None)
                    ...
                    # Button to go back to signup with unique key
                    if st.button("← Back to Sign Up", key=f"back_to_signup_{i}"):
                        st.session_state['redirect_to_login'] = False
                        st.session_state.pop('new_account_details', None)
                        st.rerun()
            return  # Exit early to avoid showing the normal tab content
                    ...
                    try:
                        ...
                        if user:
                            ...
                            with st.expander("🔍 Debug Information"):
                                try:
                                    users_df = services['auth'].storage.read_sheet("Users")
                                    # Clean user IDs for comparison
                                    clean_user_id = user_id.strip()
                                    users_df['user_id'] = users_df['user_id'].astype(str).str.strip()
                                    
                                    if clean_user_id in users_df['user_id'].values:
                                        user_row = users_df[users_df['user_id'] == clean_user_id].iloc[0]
                                        st.write(f"✅ User found in database: {user_row['user_id']}")
                                        st.write(f"   Role: {user_row['role']}")
                                        
                                        plain_password = user_row.get('plain_password', '')
                                        has_plain_password = bool(plain_password)
                                        st.write(f"   Has plain password: {has_plain_password}")
                                        
                                        if has_plain_password:
                                            st.write(f"   Stored password: {plain_password}")
                                            st.write(f"   Input password: {password}")
                                            passwords_match = password == plain_password
                                            st.write(f"   Passwords match: {passwords_match}")
                                            if not passwords_match:
                                                st.write("❌ Plain password comparison failed")
                                                st.write("🔍 Check password spelling and case sensitivity")
                                        else:
                                            st.write("❌ User has no plain password in database")
                                            st.write("💡 User may need to be recreated with plain password")
                                            
                                    else:
                                        st.write(f"❌ User ID '{clean_user_id}' not found in database")
                                        st.write("Available user IDs:")
                                        for uid in users_df['user_id'].values:
                                            st.write(f"  - '{uid}' (length: {len(uid)})")
        ...
        with tab2:
            ...
            with st.form("signup_form"):
                new_user_id = st.text_input("User ID", placeholder="Enter a unique user ID")
            ...
            if signup_submitted:
                if all([new_user_id, new_name, new_email, new_password, confirm_password]):
                    if new_password != confirm_password:
                        ...
                        try:
                            with st.spinner('🔄 Creating your account...'):
                                # Use create_account method from AuthService
                                ...
                                if success:
                                    # Store success state and account details for login redirect
                                    st.session_state['redirect_to_login'] = True
                                    st.session_state['new_account_details'] = {
                                        'user_id': new_user_id,
                                        'name': new_name,
                                        'email': new_email
                                    }
...
def render_dashboard(services, user):
    ...
    if is_user and not orders_df.empty and 'timestamp' in orders_df.columns:
        ...
        try:
            orders_df['Order Date'] = pd.to_datetime(orders_df['timestamp'], format='mixed', utc=True, errors='coerce').dt.date
            datewise_orders = orders_df.groupby('Order Date').size().reset_index(name='Order Count')
            # Remove any NaT dates that couldn't be parsed
            datewise_orders = datewise_orders.dropna(subset=['Order Date'])
            st.dataframe(datewise_orders, width='stretch', hide_index=True)
        except Exception as e:
            st.warning(f"Date parsing issue: {e}")
            st.info("Unable to display date-wise breakdown due to timestamp format issues.")
    # Date Filters
    ...
    try:
        ...
        # Use filtered data if available, otherwise fall back to full data
        display_kpi_report = filtered_kpi_report if filtered_kpi_report else full_kpi_report
        
        if display_kpi_report:
            # Core KPIs (Filtered or Full)
            core_kpis = display_kpi_report['core_order_kpis']
            daily_trends = display_kpi_report['daily_trends']
            
            # Show a message if we're falling back to full data
            if not filtered_kpi_report:
                st.info("📅 No data found for the selected date range. Showing overall analytics instead.")
            ...
            if not is_user:
                ...
                geographic_kpis = display_kpi_report['geographic_kpis']
                ...
                status_kpis = display_kpi_report['status_kpis']
...
def render_orders_page(services, user):
    ...
    with tab1:
        ...
        if user_role == 'admin':
            ...
            if not all_orders.empty and 'created_by' in all_orders.columns:
                ...
                all_orders['created_by'] = all_orders['created_by'].astype(str).str.strip()
                clean_user_id = str(user_id).strip()
                orders = all_orders[all_orders['created_by'] == clean_user_id]
        ...
        if not orders.empty:
            ...
            st.info("No orders found yet. Create your first order using the Create Order tab!")
    ...
    with tab2:
                ...
                # Manual refresh button (outside the form)
