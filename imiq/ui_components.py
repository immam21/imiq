    ...
    if user.get('role', '').lower() == 'admin':
        try:
            ...
            if not orders_df.empty and 'timestamp' in orders_df.columns:
                ...
                try:
                    orders_df['order_date'] = pd.to_datetime(orders_df['timestamp'], format='mixed', utc=True, errors='coerce').dt.date
                    today_orders = len(orders_df[orders_df['order_date'] == today])
                except Exception as e:
                    logger.warning(f"Date parsing issue in sidebar stats: {e}")
                    today_orders = 0
