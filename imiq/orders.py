...
class OrderService:
    ...
    def get_user_orders(self, user_id: str) -> pd.DataFrame:
        ...
        try:
            ...
            # Clean up created_by column to handle trailing spaces
            ...
            # Sort by timestamp, newest first - handle mixed datetime formats
            if 'timestamp' in user_orders.columns and not user_orders.empty:
                try:
                    # Use mixed format and infer_datetime_format for better parsing
                    user_orders['timestamp'] = pd.to_datetime(
                    ...
                    logger.warning(f"Timestamp parsing warning for user {user_id}: {e}")
                    # Fallback - just return without sorting if timestamp parsing fails
            
            logger.info(f"Retrieved {len(user_orders)} orders for user {clean_user_id}")
            return user_orders
